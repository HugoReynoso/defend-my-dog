...
export class SaveManager{
 ...
 static complete(level:number,stars:number){const s=this.load();const old=s.starsPerLevel[level]||0;s.starsPerLevel[level]=Math.max(old,stars);s.highestUnlockedLevel=Math.max(s.highestUnlockedLevel,Math.min(10,level+1));if(!old)s.coins+=10+(stars===3?10:0);s.tutorialCompleted=true;this.save(s);return s}
}
